...
本目录提供一套基于 TCP 传输的端到端 Postgres 入库用例，验证统一 TCP Source 与 Postgres Sink 连接器是否按预期工作。

- 发送端：`wpgen` 将样例数据通过 TCP 协议发送到指定端口（默认 19001）
- 引擎端：`wparse` 监听 TCP 端口接收数据，解析并路由到 Postgres Sink 完成入库
- 验证端：通过 Postgres 查询验证数据是否正确入库

## 数据流图

下图展示 tcp_postgres 的数据流与关键环节。
...
flowchart LR
    ...
    subgraph Database
      POSTGRES[(Postgres<br/>nginx_logs 表)]
    end
...
```
wpgen(sample) --> TCP(TCP:19001) --> wparse(daemon) --> [OML/route] --> sinks{postgres}
    sinks --> Postgres: nginx_logs 表
```
...
- `topology/sinks/business.d/all.toml`：业务 Sink 路由（包含 Postgres sink，配置入库字段）
- `models/oml/nginx.oml`：OML 模型（结果字段映射/脱敏）
- `models/wpl/nginx/`：WPL 解析规则与样例数据
- `preparatory_work.sql`：Postgres 表结构定义
- `run.sh`：一键运行脚本

说明：Source 与 Sink 连接器 id 引用仓库根目录 `connectors/` 下的定义：
- `connectors/source.d/20-tcp.toml`：id=`tcp_src`（允许覆写 `port/prefer_newline`）
- `connectors/sink.d/20-postgres.toml`：id=`postgres_sink`（允许覆写 `table/columns/dsn` 等）

## 前置要求

- 本机已启动 Postgres，默认地址 `127.0.0.1:3306`（或通过环境变量覆盖，见下文）
...
### topology/sinks/business.d/all.toml（Postgres Sink 配置）
...
```bash
postgres -h 127.0.0.1 -u root -p wparse < preparatory_work.sql
```

或直接复制 `preparatory_work.sql` 内容到 Postgres 客户端执行。

## 结果验证

- Postgres 入库验证：连接数据库查询 `nginx_logs` 表，确认记录数与数据内容
...
- **连接失败**：确认 Postgres 服务已启动，用户有目标数据库访问权限，表已创建
